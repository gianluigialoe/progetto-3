...
const risultato = whoIsBigger(7, 3);
...
const risultatoSplit = splitMe("I love coding");
...
const risultatoSenzaInizio = deleteOne("Hello", true);
const risultatoSenzaFine = deleteOne("Hello", false);
console.log(risultatoSenzaInizio);  
...
function rollTheDices(numeroDiTiri) {
  ...
  for (let i = 0; i < numeroDiTiri; i++) {
    ...
    risultato.sum += valoreDado;
    risultato.values.push(valoreDado);
  }
...
const risultatoTiri = rollTheDices(3);
...
const dataDiPartenza = '2023-01-01';
const giorniTrascorsi = howManyDays(dataDiPartenza);
console.log(`Sono trascorsi ${giorniTrascorsi} giorni dalla data di partenza.`);
...
function isTodayMyBirthday() {
  ...
  const meseCorrente = oggi.getMonth() + 1;  // Mese in JavaScript è 0-indexed, quindi aggiungi 1

 
  return giornoCorrente === 8 && meseCorrente === 9;
}


const oggiEIlTuoCompleanno = isTodayMyBirthday();
console.log('Oggi è il tuo compleanno?', oggiEIlTuoCompleanno);
...
*/
function deleteProp(oggetto, nomeProprietaDaEliminare) {
 
  delete oggetto[nomeProprietaDaEliminare];
...
const mioOggetto = {
  chiave1: 'Valore 1',
  chiave2: 'Valore 2',
  chiaveDaEliminare: 'Stringa da eliminare'
};

const oggettoModificato = deleteProp(mioOggetto, 'chiaveDaEliminare');
...
function newestMovie() {
  ...
  for (let i = 1; i < movies.length; i++) {
    ...
    if (parseInt(currentMovie.Year, 10) > parseInt(newest.Year, 10)) {
      newest = currentMovie;
    }
  }
...
const filmPiuRecente = newestMovie();
...
function onlyInLastMillennium (){
 ...
 const de= movies[i];
if(parseInt(currentMovie.Year) <2000){
  filmNuo.push(de);}}
...
function sumAllTheYears(){
  ...
  for(let i =0; i <= movies.length;i++){
    const currentMovie = movies[i];
    totalyear += parseInt(currentMovie.Year);
    return totalyear;}

 
}
const tot= sumAllTheYears()
...
*/
function  selezionaretag() {
  const tdseòe =  document.querySelectorAll('td');
}
return tdseòe;
/* ESERCIZIO 22
